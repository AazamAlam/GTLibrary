...
const EmployeeDashboard = ({ onReportClick, itemStatus, areaStatus, userType, onClearReportClick }) => {
    ...
    const [viewingNotes, setViewingNotes] = useState(''); // New state for notes to display in view-notes-modal
    const [viewingItemId, setViewingItemId] = useState(null); // New state for item whose notes are being viewed
    ...
    const hideModal = (modal) => {
        ...
        setViewingNotes(''); // Clear viewing notes on modal hide
        setViewingItemId(null); // Clear viewing item ID on modal hide
    };
    ...
    const handleClearReport = (objectId) => {
        onClearReportClick(objectId);
        hideModal(document.getElementById('clear-report-modal'));
        applyStyles(); // Re-apply styles after clearing report
    };
    ...
    const showModal = useCallback((modalId, title, data) => {
        ...
        if (modalId === 'monitor-horizontal-modal' && data && data.areaId) {
            ...
            clearButton.onclick = () => handleClearReport(data.objectId); // Use new handler
        ...
        } else if (modalId === 'view-notes-modal' && data && data.objectId && data.notes) {
            setViewingItemId(data.objectId);
            setViewingNotes(data.notes);
        }

        modal.classList.add('visible');
    }, [applyStyles, generateMonitors, handleClearReport, setReportingItemId, setViewingItemId, setViewingNotes, setCurrentModalRoomId, setSelectedRoom, setShowBookingForm, onClearReportClick]);
        ...
        const clickHandler = (event) => {
            ...
            if (interactiveElement) {
                ...
                if (userType === 'employee') {
                    if (itemStatus[objectId] && itemStatus[objectId].status === 'broken') {
                    ...
                    // Employees do not open report-issue-modal for non-broken items
                } else { // Student behavior
            ...
            if (event.target.closest('#modal-room-layout')) {
                ...
                if (roomItem) {
                    ...
                    if (userType === 'employee') {
                        ...
                        // Employees do not open report-issue-modal for non-broken room items
                    } else { // Student behavior
                        setReportingItemId(objectId);
                        showModal('report-issue-modal', `Report Issue for ${objectId}`, { objectId });
                    }
            ...
            if (event.target.matches('.interactive-item')) {
                ...
                if (userType === 'employee') {
                    if (itemStatus[itemId] && itemStatus[itemId].status === 'broken') {
                        showModal('clear-report-modal', 'Clear Report', { objectId: itemId });
                    }
                    // Employees do not open report-issue-modal for non-broken interactive items
                } else { // Student behavior
    ...
    return (
        <div className="homepage-container">
                ...
                <div className="navbar-content">
                    <div className="navbar-flex">
                        ...
                        <div className="navbar-links">
                            ...
                            <Link to="/employee-dashboard" className="navbar-link">Employee Dashboard</Link>
                    ...
                    <div className="dashboard-layout">
                        ...
                        <div className="blank-container">
                            ...
                            <div className="reported-issues-list">
                                {Object.entries(itemStatus).filter(([itemId, data]) => data.status === 'broken').length === 0 ? (
                                    <p>No issues reported.</p>
                                ) : (
                                    Object.entries(itemStatus).filter(([itemId, data]) => data.status === 'broken').map(([itemId, data]) => (
                                        <div key={itemId} className="reported-issue-item">
                                            <span>{itemId}</span>
                                            <button onClick={() => showModal('view-notes-modal', `Notes for ${itemId}`, { objectId: itemId, notes: data.notes })}>View Notes</button>
                                            <button onClick={() => handleClearReport(itemId)}>Clear Report</button>
                                        </div>
                                    ))
                                )}
                            </div>
            ...
            <div id="clear-report-modal" className="modal">
                <div className="modal-content">
                    ...
                    <div className="modal-body">
                        ...
                        <button onClick={() => handleClearReport(reportingItemId)}>Clear</button>
                    </div>
                </div>
            </div>
            ...
            <div id="view-notes-modal" className="modal">
                <div className="modal-content">
                    <span className="close-button" onClick={() => hideModal(document.getElementById('view-notes-modal'))}>&times;</span>
                    <h2>Notes for {viewingItemId}</h2>
                    <div className="modal-body">
                        <p>{viewingNotes}</p>
                    </div>
                </div>
